...
3. `/api/extract-advanced` loads local prompt/canon JSON and forwards the request to `PADDLE_OCR_URL + "/extract"`.
4. `Ingest` posts `extract_output` to `/api/ingest`.
5. `/api/ingest` keeps the existing canon prompt block and forwards the request to `PADDLE_OCR_URL + "/ingest"`.
...
- The app uses the HF OCR service for both extraction and ingest in the current flow.
